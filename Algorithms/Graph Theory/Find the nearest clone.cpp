...
int findShortest(int graph_nodes, vector<int> graph_from, vector<int> graph_to, vector<long> ids, int val) {
    ...
    vector <vector<int>> graph(graph_nodes);
    ...
    for (int clr_idx = 0; clr_idx <= ids.size(); clr_idx++) {
        if (ids[clr_idx] != val) continue;
        vector<int> ans(graph_nodes, INT_MAX);
        ans[clr_idx] = 0;
        ...
        for (int i = 0; i < graph_nodes; i++) {
            if (i == clr_idx || ids[i] != val) continue;
            shortPath = min(shortPath, ans[i]);
            finded = true;
        }

    }

    return (finded) ? shortPath : -1;
